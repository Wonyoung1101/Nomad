...
public class ActivitySelectionService {
    ...
    boolean autoSchedule) {
        ...
        ItineraryItem item = new ItineraryItem(UUID.randomUUID().toString(), activity, finalTime);

        itineraries.computeIfAbsent(userId, k -> new ArrayList<>()).add(item);
        return item;
    }

    private LocalDateTime autoAssignTime(String userId, LocalDateTime preferredTime) {
        //This is a very basic implementation of auto-scheduling logic. In a real application, this would be much more complex and would take into account the user's existing itinerary, the duration of the activity, and other factors.
        ...
        LocalDateTime latestEndTime = itinerary.stream()
                .map(item -> item.getScheduledTime().plusHours(2)) // Assuming each activity takes 2 hours
